...
@click.option(
    ...
    default=0,
    type=int,
    help=f"Skip n items from the matched items.",
)
@click.option(
    "--limit",
    "-l",
    default=10,
    type=int,
    help=f"Maximum number items in response. Maximum return size is 100 items.",
)
@click.option(
    "--query_join_operator",
    "-j",
    default="and",
    type=str,
    help=f"If multiple keywords are defined and there is no join operator (+, |) in query, One of the 'and' (default) or 'or' operator will be used.",
)
@click.option(
    "--body",
    "-b",
    default=None,
    type=str,
    help=f"Advanced filter options in json format. Please read the API documentation.",
)
@click.option(
    "--study_ids",
    "--id",
    is_flag=True,
    default=False,
    help=f"Shows only MetaboLights accession numbers.",
)
@click.option(
    "--raw",
    is_flag=True,
    default=False,
    help=f"Shows raw result in json format.",
)
