...
class DefaultStudyMetadataFileProvider(AbstractMetadataFileProvider):
    ...
    async def exists(
        ...
        file_path = Path(
            await self.get_study_metadata_path(study_id, file_relative_path)
        )
...
class AsyncMetabolightsStudyProvider:
    ...
    async def get_phase1_input_data(
        self,
        study_id: str,
        ...
        await self.update_investigation_file(model, folder, study_id=study_id)
        if self.db_metadata_collector and connection:
            logger.debug("Load %s study database metadata.", study_id)
            await self.update_study_db_metadata(
                model,
        ...
        for study_item in investigation.studies:
            ...
            for assay_item in study_item.study_assays.assays:
                file_path = await self.get_file_path(
                    assay_item.file_name, folder, study_id
                )
        ...
        for assignment_file in assignment_files:
            absolute_path = await self.get_file_path(assignment_file, folder, study_id)
            logger.debug(
    ...
    async def get_sample_file_input(
        ...
        if not model:
            model = await self.get_phase1_input_data(study_id, folder, connection)
    ...
    async def get_phase2_input_data(
        self,
        study_id: str,
        folder: Union[None, str],
        connection=None,
    ...
    async def get_phase3_input_data(
        self,
        study_id: str,
        folder: str,
        connection,
    ...
    async def get_phase4_input_data(
        self,
        study_id: str,
        folder: Union[str, None] = None,
        connection=None,
    ...
    async def get_metabolights_study_model(
        self,
        study_id: str,
        folder: str,
        connection,
    ...
    async def load_study(
        ...
        model = await self.get_phase1_input_data(study_id, study_path, connection)
        if load_sample_file and not load_assay_files and not load_folder_metadata:
            model = await self.get_sample_file_input(
                study_id,
                study_path,
                connection,
                model=model,
        ...
        if load_assay_files or load_folder_metadata:
            model = await self.get_phase2_input_data(
                study_id,
                study_path,
                connection,
                model=model,
        ...
        if load_folder_metadata:
            model = await self.get_phase4_input_data(
                study_id,
                study_path,
                connection=connection,
                model=model,
        ...
        if load_maf_files:
            model = await self.get_phase3_input_data(
                study_id,
                study_path,
                connection,
                model=model,
    ...
    async def update_investigation_file(
        self,
        model: MetabolightsStudyModel,
        folder,
        ...
        file = await self.get_file_path(file_name, folder, study_id)
    ...
    async def update_study_db_metadata(
        self,
        model: MetabolightsStudyModel,
        ...
        try:
            study_db_metadata, messages = await collector.get_study_metadata_from_db(
                study_id, connection
            )
            if messages:
