...
def get_investigation(
    ...
    process_study_comments(investigation)
    process_study_design_descriptor_comments(investigation)
    process_study_assay_comments(investigation)
    process_study_contact_comments(investigation)
    return investigation


def process_study_design_descriptor_comments(investigation: Investigation):
    if investigation.studies:
        ...
        for study in investigation.studies:
            if study.study_design_descriptors.comments:
                ...
                for (
                    comment_name,
                    field_name,
                ) in design_descriptor_comment_field_map.items():
...
def process_study_contact_comments(investigation: Investigation):
    if investigation.studies:
        for study in investigation.studies:
            if study.study_contacts.comments:
                comments_dict: dict[str, list[str]] = {}
                ...
                additional_emails = comments_dict.get(
                    "study person additional email", []
                )
...
def process_study_assay_comments(investigation: Investigation):
    if investigation.studies:
        for study in investigation.studies:
            if study.study_assays.comments:
                comments_dict: dict[str, list[str]] = {}
                ...
                assay_type_term_source = comments_dict.get(
                ...
                omics_type_term_source = comments_dict.get(
                ...
                assay_descriptor_source = comments_dict.get(
...
def process_study_comments(investigation: Investigation):
    if investigation.studies:
        ...
        for study in investigation.studies:
            if study.comments:
                ...
                max_grant_size = max(
                    [len(x) for x in (funders, funder_ids, grant_ids) if x and x[0]]
                )
                if max_grant_size > 0:
                    study.funders = []
                    for idx in range(max_grant_size):
                        study.funders.append(
                            Funder(
                                funder_name=funders[idx] if len(funders) > idx else "",
                                funder_id=funder_ids[idx]
                                if len(funder_ids) > idx
                                else "",
                ...
                characteristic_names = comments_dict.get(
                    "study characteristics name", []
                )
                characteristic_types = comments_dict.get(
                ...
                if max_characteristic_types > 0:
                    study.characteristic_types = []
                    for idx in range(max_characteristic_types):
