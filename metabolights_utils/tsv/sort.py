...
class TsvFileSortOption(CamelCaseModel):
    column_name: str = Field(
        ...
        "If there are multiple column headers, Second column and others are named as <column header>.1 <column header>.2 and so on.",
    )
    reverse: bool = Field(False, description="Sort descending order")
    column_sort_type: SortType = Field(
        SortType.STRING, description="Sort value with the given type"
    )
    case_sensitive: bool = Field(
        True, description="Sort string values in case sensitive order"
    )
    default_datetime_pattern: str = Field(
        "%d/%m/%Y", description="Convert string value to datetime using this pattern"
    )
    value_order: TsvFileSortValueOrder = Field(
    ...
    custom_sorter_name: str = Field("", description="Registered custom sorter name")
    custom_sorter_arguments: Dict[str, Any] = Field(
        {}, description="Additional arguments for custom sorters"
    )
