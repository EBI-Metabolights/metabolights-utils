...
def test_add_row_action_01():
    ...
    delete_row_action: TsvDeleteRowsAction = TsvDeleteRowsAction(current_row_indices=[8171, 8172])
    result: TsvActionReport = isa_table_updater.apply_actions(
        file_path=path,
...
def test_move_row_action_01():
    ...
    os.makedirs(os.path.dirname(path), exist_ok=True)
    shutil.copy(path_original, path)
    move_row_action: TsvActionReport = TsvMoveRowAction(source_row_index=4, new_row_index=1)
    result: TsvActionReport = isa_table_updater.apply_actions(
        file_path=path,
...
def test_move_row_action_02():
    ...
    os.makedirs(os.path.dirname(path), exist_ok=True)
    shutil.copy(path_original, path)
    move_row_action: TsvActionReport = TsvMoveRowAction(source_row_index=10, new_row_index=0)
    result: TsvActionReport = isa_table_updater.apply_actions(
        file_path=path,
        file_sha256_hash=sha,
        actions=[move_row_action],
...
def test_move_row_action_03():
    ...
    os.makedirs(os.path.dirname(path), exist_ok=True)
    shutil.copy(path_original, path)
    move_row_action: TsvActionReport = TsvMoveRowAction(source_row_index=2, new_row_index=8166)
    result: TsvActionReport = isa_table_updater.apply_actions(
        file_path=path,
        file_sha256_hash=sha,
        actions=[move_row_action],
...
def test_move_row_action_04():
    ...
    os.makedirs(os.path.dirname(path), exist_ok=True)
    shutil.copy(path_original, path)
    move_row_action: TsvActionReport = TsvMoveRowAction(source_row_index=8166, new_row_index=20)
    result: TsvActionReport = isa_table_updater.apply_actions(
        file_path=path,
        file_sha256_hash=sha,
        actions=[move_row_action],
...
def test_move_row_action_05():
    ...
    os.makedirs(os.path.dirname(path), exist_ok=True)
    shutil.copy(path_original, path)
    move_row_action: TsvActionReport = TsvMoveRowAction(source_row_index=20, new_row_index=21)
    result: TsvActionReport = isa_table_updater.apply_actions(
        file_path=path,
        file_sha256_hash=sha,
        actions=[move_row_action],
...
def test_move_row_action_06():
    ...
    os.makedirs(os.path.dirname(path), exist_ok=True)
    shutil.copy(path_original, path)
    move_row_action: TsvActionReport = TsvMoveRowAction(source_row_index=25, new_row_index=24)
    result: TsvActionReport = isa_table_updater.apply_actions(
        file_path=path,
        file_sha256_hash=sha,
        actions=[move_row_action],
...
def test_move_row_action_07():
    ...
    os.makedirs(os.path.dirname(path), exist_ok=True)
    shutil.copy(path_original, path)
    move_row_action: TsvActionReport = TsvMoveRowAction(source_row_index=11, new_row_index=11)
    result: TsvActionReport = isa_table_updater.apply_actions(
        file_path=path,
        file_sha256_hash=sha,
        actions=[move_row_action],
...
def test_move_column_action_01():
    ...
    move_column_action: TsvActionReport = TsvMoveColumnAction(
        source_column_index=1, source_column_header="Characteristics[Organism]", new_column_index=6
    )
    result: TsvActionReport = isa_table_updater.apply_actions(
        file_path=path,
        file_sha256_hash=sha,
...
def test_update_column_action_01():
    ...
    columns[1] = column
    column = TsvColumnData(
        header_name="Characteristics[Organism part]", values={2: "test 2.2", 5: "test 2.5"}
    )
    columns[4] = column
