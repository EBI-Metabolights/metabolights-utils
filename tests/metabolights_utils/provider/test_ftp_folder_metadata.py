...
def folder_index_file_path() -> (
    ...
    folder_index_file_path = (
        f"/tmp/mtbls_folder_index_file_path_{random.randint(1000000, 9999999)}_tmp"
    )
    try:
...
def test_get_folder_metadata_01(folder_index_file_path: str, study_id: str):
    ...
    mock_client.remote_repository_root_directory.return_value = "/tmp/path"
    ...
    # reuse same file
    with open(folder_index_file_path, "w") as f:
