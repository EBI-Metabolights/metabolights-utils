...
def test_public_search_04(mocker: MockerFixture):
    ...
    runner = CliRunner()
    result = runner.invoke(public_search, [f"diet"])
    assert result.exit_code == 0
    assert f"MetaboLights public study search results for query: diet" in result.output

    result = runner.invoke(public_search, [f"diet", "--id"])
    assert result.exit_code == 0
    assert f"MTBLS225" in result.output

    result = runner.invoke(public_search, [f"diet", "--raw", "--limit=1"])
    assert result.exit_code == 0
    assert f"MTBLS225" in result.output
...
def test_public_search_05(mocker: MockerFixture):
    ...
    runner = CliRunner()
    result = runner.invoke(public_search, [f"dietdiet", "--id"])
    assert result.exit_code == 0
    assert "[]" in result.output

    result = runner.invoke(public_search, [f"dietdiet"])
    assert result.exit_code == 0
...
def test_public_search_08(mocker: MockerFixture):
    ...
    runner = CliRunner()
    result = runner.invoke(public_search, [f"diet", "--body=diet"])
    assert result.exit_code == 1
    # assert "Invalid response." in result.output
